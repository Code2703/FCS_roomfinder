...
def map():
    ...
    if start_room_nr is not None and dest_room_nr is not None:
        ...
        dest_poiId_query = rooms.query('room_nr == @dest_room_nr')['poiId']
        if not dest_poiId_query.empty:
            dest_poiId = dest_poiId_query.iloc[0]
        else:
            # Geeignete Fehlerbehandlung oder alternativer Ablauf
            dest_poiId = None  # oder eine andere Standardaktion
